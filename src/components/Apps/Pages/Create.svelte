...
<script>
  ...
  let appId
    ...
    let esSave = await scripts.app.pages.getSave(appId, token, change)
    console.log('esSave', esSave)
    if (esSave.payload.success === true) {
      window.location.href = `/apps/${domain}/${state}/pages/${slug}`
    } else {
...
<div class="row">
  ...
  <div class="col s12 m4">
    ...
    <div style="text-align: right;">
      <a href={`/apps/${domain}/${state}/pages`} class="waves-effect red lighten-2 btn">Cancel</a>
    </div>
  </div>
  <div class="col s12 m4"></div>
</div>
