...
<div class="row">
  ...
  <div class="col s12 m10">
    ...
    <div class="card">
      <a href={`/apps/${domain}/${state}/guides/add`} class="floating-add btn-floating btn-large waves-effect waves-light"><i class="material-icons">add</i></a> 
      <div class="list">
        {#if table.length}
            ...
            <tbody>
              {#each $rows as row}
                <tr>
                  ...
                  <td>{row.name}</td>
                  <td><a href={`https://${endpoint}.burnfort.com/guides/${row.slug}`} target="_blank">/guides/{row.slug}</a></td>
                  <td style="text-align: right;"><a href={`/apps/${domain}/${state}/guides/${row.slug}`} class="btn  waves-effect waves-light"><i class="material-icons">edit</i></a></td>
                </tr>
              {/each}
            </tbody>
          </Datatable>
