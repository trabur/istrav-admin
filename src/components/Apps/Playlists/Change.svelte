...
<script>
  ...
  onMount(async () => {
    ...
    if (esOne.payload.success === true) {
      ...
      let esPlaylist = await scripts.channel.playlists.getOne(appId, slug)
      console.log('esPlaylist', esPlaylist)
      if (esPlaylist.payload.success === true) {
        let data = esPlaylist.payload.data
        name = data.name
        slug = data.slug
        image = data.image
        setTimeout(() => M.updateTextFields(), 0)
      } else {
        alert(esPlaylist.payload.reason)
      }
    } else {
