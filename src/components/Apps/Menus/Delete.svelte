...
<script>
  ...
  function areYouSure() {
    var confirmDelete = prompt(`Are you sure that you want to delete the "${slug}" menu? Please renter quotation to confirm.`, "");
    if (confirmDelete === `${slug}`) {
      alert('Confirm delete success. The menu will be removed.')
      remove()
    } else {
      alert('Confirm delete mismatch. The menu will not be removed.')
    }
  }
  ...
  async function remove() {
    ...
    if (esDelete.payload.success === true) {
      window.location.href = `/apps/${domain}/${state}/menus`
    } else {
